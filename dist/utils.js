...
const seedDB = async () => {
    try {
        ...
        await User.findOneAndUpdate({ username: 'Bradley_Santiago' }, { $push: { thoughts: thoughts[0]._id } });
        await User.findOneAndUpdate({ username: 'Edward_Apostol' }, { $push: { thoughts: thoughts[1]._id } });
        await User.findOneAndUpdate({ username: 'Thais_Cadet' }, { $push: { thoughts: thoughts[2]._id } });
